...
function default_1(routes) {
    ...
    function createRouteRecoed(route, parent) {
        ...
        if (route.children) {
            route.children.map(function ($route) { return createRouteRecoed($route, record); });
        }
    }
}
...
function matchRecord(path, routeRecord) {
    ...
    try {
        for (var _b = __values(routeRecord.values()), _c = _b.next(); !_c.done; _c = _b.next()) {
            var record = _c.value;
            var matchVal = record.regex.exec(path);
            if (matchVal) {
                var params = {};
                ...
                return { params: params, componentNameList: componentNameList, path: path };
            }
        }
    }
    catch (e_1_1) { e_1 = { error: e_1_1 }; }
...
function default_1(config) {
    ...
    this[namespace] = {
        ...
        //方法
        get go() {
            return __router.go.bind(__router);
        },
        get push() {
...
var Router = /** @class */ (function () {
    ...
    Router.prototype.go = function (n) {
        var doIt = false;
        if (n > 0 && this.currentPathIndex + n <= this.histry.length) {
            // 前进
            doIt = true;
        }
        else if (n < 0 && this.currentPathIndex + n > 0) {
            // 后退
            doIt = true;
        }
        if (doIt) {
            var newCurrentPathIndex = this.currentPathIndex + n;
            var path = this.histry[newCurrentPathIndex - 1];
            if (!this.__invokeBeforeHooks(path))
                return;
            this.__matchrouteRecord(path);
            this.__setCurrentPath(path);
            this.currentPathIndex = newCurrentPathIndex;
    ...
    Router.prototype.replace = function (path) {
        if (!this.__invokeBeforeHooks(path))
            return;
        this.__matchrouteRecord(path);
        this.histryReplace(path);
        this.__setCurrentPath(path);
        this.__invokeAfterHooks && this.__invokeAfterHooks();
    };
    Router.prototype.histryPush = function (path) {
        if (this.histry.length !== this.currentPathIndex) {
            this.histry.splice(this.currentPathIndex);
        }
        this.currentPathIndex += 1;
        this.histry.push(path);
    };
    Router.prototype.histryReplace = function (path) {
        this.histry.splice(this.currentPathIndex - 1, 1, path);
    };
